...
namespace RunSection
{
    ...
    //void BasicTask::SCTimeIntegration(SpinAPI::system_ptr& sysptr, SpinAPI::SpinSpace& space, arma::sp_cx_mat& A, SCData& Data, arma::cx_vec& rho0vec, TimeEvoProperties& prop)
    //{
	//	std::vector<std::pair<int,arma::cx_vec>> SCresults;
	//	std::vector<std::pair<int,double>> SCweights;
	//	std::vector<arma::sp_cx_mat> As;
...
//
	//	{
	//		for(auto e = sysptr->interactions_cbegin(); e != sysptr->interactions_cend(); e++)
	//		{
	//			if((*e)->Type() == SpinAPI::InteractionType::SemiClassicalField)
	//			{
	//				AllWeights[0].push_back((*e)->GetOriWeights());
...
//
	...
	//	
	//	arma::cx_mat PState;
	//	auto states = sysptr->States();
	//	for (auto j = states.cbegin(); j != states.cend(); j++)
	//	{
	//		if (!space.GetState((*j), PState))
	//		{
	//			this->Log() << "Failed to obtain projection matrix onto state \"" << (*j)->Name() << "\" of SpinSystem \"" << (*sysptr)->Name() << "\"." << std::endl;
	//			continue;
	//		}
